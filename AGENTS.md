...
- `scripts/weapons/weapon.gd`: base weapon; creates bullets in code and attaches `scripts/weapons/bullet.gd`. `damage_min`/`damage_max` (both -1 by default) let a weapon roll random per-shot damage in `shoot()`; falls back to the flat `damage` export when `damage_max <= damage_min`.
- `scripts/weapons/pistol.gd`: pistol stats via `Weapon` inheritance — sets `damage_min = 7.0`/`damage_max = 9.0` (random damage per shot), not a flat `damage`.
