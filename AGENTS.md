...
- `scripts/world/hud.gd`'s threat-density indicator (top-left `%DensidadeAmeacaLabel` in `camada_ui.tscn`, icon reused from `resources/sprites/characters/ameaca.png`'s existing idle-frame `AtlasTexture` region) counts living `Ameaca` by walking `get_tree().get_nodes_in_group(&"ameacas")` every `_process` frame — deliberately NOT via `ZonaPopulador`'s snapshot, which only updates on scene transition/save and would show a stale count while enemies die mid-visit.
