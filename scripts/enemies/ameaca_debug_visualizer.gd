...
func _draw() -> void:
	if not debug_draw_enabled or not is_instance_valid(ameaca) or ameaca._is_dead:
		return
	...
	if ameaca.has_direct_vision_to_player() and is_instance_valid(player):
		target_pos = player.global_position
		target_label = "JOGADOR (VISAO DIRETA)"
		target_color = Color(0.2, 1.0, 0.3)
	elif ameaca._has_sound_target:
		target_pos = ameaca._sound_target_pos
		target_label = "SOM (RUIDO)"
		target_color = Color(1.0, 0.85, 0.1)
	elif ameaca._is_investigating_last_seen:
		target_pos = ameaca._last_seen_player_pos
		target_label = "ULTIMO LOCAL VISTO"
		target_color = Color(0.5, 0.8, 1.0)
	...
	if font != null:
		...
		if ameaca.has_direct_vision_to_player():
